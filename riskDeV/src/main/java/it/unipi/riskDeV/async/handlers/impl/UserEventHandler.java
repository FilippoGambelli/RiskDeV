...
public class UserEventHandler implements EventHandler {
    ...
    public Result<Void> handle(String payloadJson) {
        ...
        log.debug("DLQ Retry: Handling project event for {}", event.username());

        return switch (event) {           
            case UserEvent.UserUpdated u -> 
                projectService.changeCollaboratorDataInProjects(u.projectNames(), u.username(), u.newUsername(), u.newEmail());
            case UserEvent.UserDeleted d -> 
                projectService.removeCollaboratorFromProjects(d.projectNames(), d.username());
        };
    }
